...
class UIMaker: # UI 라이브러리 사용
    ...
    def printMenu(self, menuList):
        ...
        for item in menuList: #menu list = (pno, name, imagePath, price)
            ...
            Button(menuFrame, text="담기", width=9, height=7, relief='ridge', bd=2, command=lambda i=item.pno: self.interface.makeOrder(orderNumber, i)).pack(side="right", fill='both')
        ...
        Button(menuWindow, text="결제", width=6, height=1, relief='ridge', bd=2, command=lambda : self.interface.makeOrder(orderNumber, 0)).pack(side="bottom", fill='both')
        menuWindow.mainloop()
        return orderNumber
    ...
    def printPayConfirm(self, totalPrice, chargedMoney):
        ...
        Button(buttonFrame, width=2,height=2,text="Y",command= lambda : self.interface.confirmCheck('Y',result)).pack(side="left")
        Button(buttonFrame, width=2, height=2, text="N",command= lambda : self.interface.confirmCheck('N',result)).pack(side="right")
    ...
    def printFinish(self):
        ...
        Button(noticeWindow, width=2, height=2, text="확인", command= lambda : noticeWindow.destroy()).pack(side="bottom", fill="both")
        menuWindow.mainloop()
    ...
    def printRecharge(self):
        ...
        Button(buttonFrame, width=2, height=2, text="Y", command=lambda: self.interface.rechargeConfirmCheck('Y', result)).pack(side="left")
        Button(buttonFrame, width=2, height=2, text="N", command=lambda: self.interface.rechargeConfirmCheck('N', result)).pack(side="right")
        buttonFrame.pack(side="bottom")
        menuWindow.mainloop()
